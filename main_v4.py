...
async def save_database_async(filename, data, yield_every=15):
    """Same as save_database, but yields to the event loop every N records
    so a big history/session/inventory file doesn't block RFID scans or
    HTTP responses while it's being written."""
    tmp_name = filename + ".tmp"
    bak_name = filename + ".bak"
    try:
        with open(tmp_name, "w") as file:
            if isinstance(data, list):
                ...
                for i, (k, v) in enumerate(data.items()):
                    if i:
                        file.write(",")
                    file.write(json.dumps(k) + ":" + json.dumps(v))
                    if i % yield_every == 0:
                        await asyncio.sleep_ms(0)
                file.write("}")
            else:
...
async def db_flush_task():
    global _dirty_files, _db_flush_timer, _last_gc_ms
    while True:
        ...
        if _dirty_files and time.ticks_diff(time.ticks_ms(), _db_flush_timer) > 0:
            ...
            for fname, data in pending.items():
                await save_database_async(fname, data)
                await asyncio.sleep_ms(10) 
            now = time.ticks_ms()
            if time.ticks_diff(now, _last_gc_ms) > GC_MIN_INTERVAL_MS:
                gc.collect()
                _last_gc_ms = now
...
def get_header_value(headers_str, header_name):
...
def generate_session_token():
    return ubinascii.hexlify(os.urandom(16)).decode()
...
def get_history_filename():
    ...
    return f"history_{y}_{m:02d}_{d:02d}.json"
...
def check_day_rollover():
    """Swaps history_db/session_history_db to a new file once the calendar
    day changes. Keeping this on a daily boundary (instead of weekly) bounds
    how many entries stay resident in RAM and how large each save-to-flash
    rewrite gets, since save_database_async rewrites the full list on every
    flush."""
...
def _parse_hms(t):
    ...
    while len(parts) < 3:
        parts.append(0)  # handles old HH:MM entries with no seconds
    h, m, s = parts[0], parts[1], parts[2]
    return h * 3600 + m * 60 + s

def format_duration(login_time, logout_time):
    if not login_time or not logout_time:
        return ""
    try:
        total_seconds = _parse_hms(logout_time) - _parse_hms(login_time)
    except (ValueError, AttributeError):
        return ""
    if total_seconds < 0:
        total_seconds += 24 * 3600  # session crossed midnight
...
async def dashboard_carousel_task():
    ...
    while True:
        ...
        request_dashboard_render()
        gc.collect()
        # NOTE: memory stats are available on demand via GET /api/diagnostics.
        # A print() here goes out over USB-CDC; if nothing is actively reading
        # that serial port, the write can block and freeze the whole event
        # loop (web server + RFID scanner included), since uasyncio is
        # single-threaded/cooperative. Do not log here every cycle.
...
# ESP32's lwIP stack only has a handful of TCP control blocks to hand out,
# and every response here is sent with "Connection: close" (a brand new
# TCP connection per request/poll). Capping concurrent *requests* alone
# isn't enough: even a request we turn away with a quick 503 still opens
# and tears down a TCP connection, so once enough distinct devices are
# polling, the connection churn itself exhausts the socket pool again.
#
# Instead we reserve a fixed number of slots keyed by client IP. A device
# that already holds a slot is always let through (its slot is refreshed).
# A brand new device only gets a slot if one is free; otherwise its
# connection is closed immediately, before we even read its request, so
# it costs as little socket/heap time as possible and can't disrupt the
# devices already using the system. Idle slots are reclaimed after
# DEVICE_IDLE_TIMEOUT_MS so a device that disconnects (closed tab, powered
# off) doesn't hold its seat forever.
MAX_ACTIVE_DEVICES = 3
DEVICE_IDLE_TIMEOUT_MS = 45000
_active_devices = {}  # client_ip -> last_seen_ms
...
def _admit_device(client_ip):
    """True if this device may use the server right now. Devices that
    already hold a slot are always admitted (and their slot refreshed).
    New devices are admitted only while a slot is free."""
    global _active_devices
    now = time.ticks_ms()

    if client_ip is None:
        # Couldn't identify the peer - don't block it, but don't track a
        # slot for it either (falls back to the request-level cap below).
        return True

    for ip in list(_active_devices.keys()):
        if time.ticks_diff(now, _active_devices[ip]) > DEVICE_IDLE_TIMEOUT_MS:
            del _active_devices[ip]

    if client_ip in _active_devices:
        _active_devices[client_ip] = now
        return True
...
# Secondary safety net: caps how many requests are processed AT ONCE
# (a single admitted device can still fire off a few parallel fetches on
# page load). This is intentionally sized off MAX_ACTIVE_DEVICES rather
# than a fixed number - if it's ever smaller than roughly
# "devices x typical parallel requests per device", it becomes the real
# bottleneck and starts rejecting requests from devices that already hold
# a valid slot, which looks exactly like the device cap not working even
# though it's this cap doing the blocking. 3 in-flight requests per
# admitted device is generous headroom for a page load's worth of
# parallel fetches.
REQUESTS_PER_DEVICE_HEADROOM = 3
MAX_CONCURRENT_CLIENTS = MAX_ACTIVE_DEVICES * REQUESTS_PER_DEVICE_HEADROOM
_active_clients = 0
...
async def handle_client(reader_stream, writer_stream):
    ...
    # Check device admission FIRST, before reading anything - a rejected
    # connection should be closed as fast as possible so it doesn't
    # linger and compete with the devices that already have a slot.
    client_ip = _get_client_ip(writer_stream)
    if not _admit_device(client_ip):
        try:
            writer_stream.write(
                b"HTTP/1.1 503 Service Unavailable\r\nConnection: close\r\n"
                b"Content-Type: text/plain\r\nContent-Length: 32\r\n\r\n"
                b"Maximum number of devices in use"
            )
    ...
    if _active_clients >= MAX_CONCURRENT_CLIENTS:
        # Fail fast instead of accepting yet another connection that we
        # don't have the sockets/heap to actually service.
        try:
            writer_stream.write(b"HTTP/1.1 503 Service Unavailable\r\nConnection: close\r\nContent-Length: 0\r\n\r\n")
            await writer_stream.drain()
        except Exception:
            pass
        try:
    ...
    try:
        try:
            request_bytes = await asyncio.wait_for(reader_stream.read(2048), 5)
        ...
        if method == 'POST':
            ...
            MAX_BODY_SIZE = 8192  # guard against a runaway allocation under memory pressure
            if content_length > MAX_BODY_SIZE:
                await send_response(writer_stream, "413 Payload Too Large", "application/json", '{"error": "body too large"}')
                return

            while len(body_bytes) < content_length:
                try:
                    chunk = await asyncio.wait_for(reader_stream.read(content_length - len(body_bytes)), 5)
                except asyncio.TimeoutError:
                    break
                if not chunk:
                    break
        ...
        req_admin_token = get_header_value(headers, "X-Admin-Token")
        is_authed_admin = admin_session_active and admin_session_token is not None and req_admin_token == admin_session_token

        # --- ROUTER ---
        if path == '/':
            await send_file_response(writer_stream, 'index.html', 'text/html')
            
        elif path == '/api/update_admin_creds' and method == 'POST' and is_authed_admin:
            try:
                req = json.loads(body)
        ...
        elif path == '/api/get_settings' and is_authed_admin:
            await send_response(writer_stream, "200 OK", "application/json", json.dumps(system_settings))
            
        elif path == '/api/update_timeouts' and method == 'POST' and is_authed_admin:
            try:
                req = json.loads(body)
        ...
        elif path == '/api/toggle_buzzer' and method == 'POST' and is_authed_admin:
            try:
                req = json.loads(body)
        ...
        elif path == '/api/diagnostics' and is_authed_admin:
            uptime_ms = time.ticks_ms()
            uptime_sec = uptime_ms // 1000
            now = time.ticks_ms()

            device_list = [
                {"ip": ip, "idle_sec": time.ticks_diff(now, last_seen) // 1000}
                for ip, last_seen in _active_devices.items()
            ]
            ...
            data = {
                ...
                "max_concurrent_requests": MAX_CONCURRENT_CLIENTS,
                "active_requests": _active_clients
            }
            await send_response(writer_stream, "200 OK", "application/json", json.dumps(data))

        elif path == '/api/reboot' and method == 'POST' and is_authed_admin:
        ...
        elif path == '/api/backup' and is_authed_admin:
        ...
        elif path == '/api/restore' and method == 'POST' and is_authed_admin:
            try:
        ...
        elif path == '/api/purge_history' and method == 'POST' and is_authed_admin:
            import os
        ...
        elif path == '/api/factory_reset' and method == 'POST' and is_authed_admin:
            import os
        ...
        elif path == '/api/edit_engineer' and method == 'POST' and is_authed_admin:
            try:
                req = json.loads(body)
            ...
            try:
                ...
                if not username or not password:
                    ...
                    await send_response(writer_stream, "409 Conflict", "application/json", '{"error": "admin already logged in"}')
        ...
        elif path == '/api/stage_registration' and method == 'POST' and is_authed_admin:
            try:
                data = json.loads(body)
        ...
        elif path == '/api/force_return' and method == 'POST' and is_authed_admin:
            try:
                req = json.loads(body)
                tool_id = req.get("id")
        ...
        elif path == '/api/bulk_action' and method == 'POST' and is_authed_admin:
            try:
                req = json.loads(body)
                action = req.get("action")
                ids = req.get("ids", [])
        ...
        elif path == '/api/delete' and method == 'POST' and is_authed_admin:
            try:
        ...
        elif path == '/api/delete_engineer' and method == 'POST' and is_authed_admin:
            try:
                del_req = json.loads(body)
                del_id = del_req.get("id")
                if not del_id:
            ...
            check_day_rollover()
            await send_response(writer_stream, "200 OK", "application/json", json.dumps({"month": "Today", "entries": history_db}))

        elif path == '/api/session_history':
            check_day_rollover()
            await send_response(writer_stream, "200 OK", "application/json", json.dumps({"month": "Today", "entries": session_history_db}))

        elif path == '/api/list_days':
            check_day_rollover()
            await save_database_async(current_history_filename, history_db)
            try:
                files = [f for f in os.listdir() if f.startswith('history_') and f.endswith('.json') and not f.endswith('.bak') and not f.endswith('.tmp')]
                days = sorted([f[len('history_'):-len('.json')] for f in files], reverse=True)
            except OSError:
                days = []
            await send_response(writer_stream, "200 OK", "application/json", json.dumps({"days": days}))

        elif path == '/api/export_history':
            check_day_rollover()
            await save_database_async(current_history_filename, history_db)
            day_filter = query_params.get('day')
            fname_suffix = f"_{day_filter}" if day_filter else ""
            headers = [
                "HTTP/1.1 200 OK",
            ...
            try:
                if day_filter:
                    candidate = f"history_{day_filter}.json"
            ...
            check_day_rollover()
            await save_database_async(current_session_filename, session_history_db)
            day_filter = query_params.get('day')
            fname_suffix = f"_{day_filter}" if day_filter else ""
            headers = [
                "HTTP/1.1 200 OK",
            ...
            csv_header = "Control No.,Name,Date,Login Time,Item,Item Time,Item Status,Logout Time,Duration\n"
            writer_stream.write(csv_header.encode('utf-8'))
            await writer_stream.drain()

            try:
                if day_filter:
                    candidate = f"sessions_{day_filter}.json"
                    files = [candidate] if candidate in os.listdir() else []
                else:
                        ...
                        with open(f, "r") as hf:
                            ...
                            if isinstance(data, list):
                                for s in data:
                                    ...
                                    for it in items:
                                        duration = format_duration(s.get('login_time',''), s.get('logout_time',''))
                                        row = f"{s.get('control_no','')},{s.get('borrowed_by','')},{s.get('date','')},{s.get('login_time','')},{it.get('name','')},{it.get('time','')},{it.get('status','')},{s.get('logout_time','')},{duration}\n"
                                        writer_stream.write(row.encode('utf-8'))
                                        await writer_stream.drain()
                    except:
                        pass
...
async def main():
    ...
    # backlog=8 gives some headroom for a browser opening several parallel
    # requests at once (e.g. the admin panel loading inventory/engineers/
    # settings together) on top of a second client polling in parallel.
    await asyncio.start_server(handle_client, '0.0.0.0', 80, backlog=8)
